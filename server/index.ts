...
app.post("/api/assignments/source-document", (req, res, next) => {
    upload.single("document")(req, res, error => {
        if (error instanceof multer.MulterError && error.code === "LIMIT_FILE_SIZE") {
            res.status(400).send(
                "The document is too large. Upload a file smaller than 10 MB."
            );
            return;
        }
...
app.post("/api/assignments/generate", async (req, res) => {
    ...
    const teacherNotes =
        typeof req.body.teacherNotes === "string" ? req.body.teacherNotes.trim() : "";
    ...
    const generated = await generateAssignmentRemixes({
        title,
        sourceText,
        teacherNotes
    });
...
app.listen(port, "0.0.0.0", () => {
    console.log(
        `Lesson Remix Studio API listening on http://0.0.0.0:${port} (LAN: use this machine's IP)`
    );
});
